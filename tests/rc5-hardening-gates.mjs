...
const checks = [
  ...
  [state.includes('if (role === "Admin") return adminChangesValid(previous, next, actorUsername);'), "Admin controlled approval path wired"],
  [state.includes('record.kind === "Adjustment"') && state.includes('record.status === "Pending Admin"') && state.includes('before.requestedBy !== actorUsername'), "Stock Controller cannot self-approve adjustments"],
